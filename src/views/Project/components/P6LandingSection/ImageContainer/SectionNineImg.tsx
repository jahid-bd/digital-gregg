...
const SectionNineImg: React.FC<Props> = () => {
  useLayoutEffect(() => {
    window.requestAnimationFrame(() => {
      const ProjectImgTl = gsap.timeline({
        scrollTrigger: {
          trigger: `._RapChatSectionNineTrigger`,
          start: '`top 100%',
          scrub: 0.9,
        },
      });
      ProjectImgTl.to(
        `._RapChatSectionNineImg`,
        {
          y: -150,
          duration: 0.8,
          ease: 'none',
  ...
  return (
    <div>
      <div className={`lg:block hidden lg:relative lg:p-[9.14286rem_4.53782%]`}>
        ...
        ></div>
        <div
          className={`mb-0 min-h-[62vw] flex justify-around _RapChatSectionNineTrigger w-[50vw] mx-auto`}
        >
          <div
            className={`min-h-full translate-y-[2.44267%] flex-grow shrink relative`}
          >
            <div className={`min-h-full _RapChatSectionNineImg`}>
              <div className="shadow-[0_0_26px_0_rgb(0_0_0_/_10%)] rounded-[0.42857rem] relative">
                ...
                <figure className="m-0 relative pt-[2.5rem]">
                  <img
                    src="/images/project/project-6/image-10.png"
                    alt="image"
