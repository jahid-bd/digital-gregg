...
const VideoContainer = ({ data, isVimeoSrc }: Props) => {
  ...
  useEffect(() => {
    window.requestAnimationFrame(() => {
      const ProjectVideoTl = gsap.timeline({
        scrollTrigger: {
          trigger: '._projectRapChat8VideoTrigger',
          start: '`top 100%',
          scrub: 0.9,
        },
      });
      ProjectVideoTl.to(
        '._projectRapChatVideo',
        {
          y: -150,
          duration: 0.8,
        },
  ...
  return (
    <div>
      <div className="bg-[#F5F5F5] lg:p-[5rem_7rem] _projectRapChat8VideoTrigger">
        <div className="overflow-hidden hidden lg:block rounded-[0.35714rem] !relative top-32 mt-5 h-full _projectRapChatVideo shadow-[0_0_26px_0_rgb(0_0_0_/_20%)] w-[50vw] mx-auto">
          ...
          <div>
            {isVimeoSrc ? (
              ...
              <video
                ref={videoModal}
                className={classNames(
                  'h-full lg:w-[70%] w-[90%] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'
                )}
              >
