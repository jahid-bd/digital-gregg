...
const OnekeyVideo3 = ({ data, isVimeoSrc }: Props) => {
  // States
  const [isVideoModalClose, setIsVideoModalClose] = useState<boolean>(false);
  ...
  // Jotai
  const [projectDetailsVideoModal, setProjectDetailsVideoModal] =
    useState<boolean>(false);

  // Use Effects
  useEffect(() => {
    projectDetailsVideoModal
  ...
  const handleOpenVideo = () => {
    setProjectDetailsVideoModal(true);
    setIsVideoModalClose(false);
  ...
  const handleCloseVideo = () => {
    setProjectDetailsVideoModal(false);
    setIsVideoModalClose(true);
    if (isVimeoSrc) {
      setIsPlay(false);
    } else {
  ...
  useEffect(() => {
    window.requestAnimationFrame(() => {
      const ProjectVideoTl = gsap.timeline({
        scrollTrigger: {
          trigger: '._windenprojectVideoTrigger3',
          start: '`top 100%',
          scrub: 0.9,
        },
      });
      ProjectVideoTl.to(
        '._windenprojectVideo3',
        {
          y: -150,
          duration: 0.8,
        },
  ...
  return (
    <>
      <div className="bg-[#F5F5F5] lg:p-[5rem_7rem] _windenprojectVideoTrigger3">
        <div className="overflow-hidden hidden lg:block rounded-[0.35714rem] !relative top-32 mt-5 h-full _windenprojectVideo3 shadow-[0_0_26px_0_rgb(0_0_0_/_20%)] w-[50vw] mx-auto">
        ...
        <div className="absolute top-0 left-0 right-0 w-full flex justify-between">
          <div className="h-[70px] relative z-[999] aspect-square flex items-center justify-center">
            <img
              ...
              alt="logo svg image"
              height={'100%'}
              className="relative z-[999] w-[32px] lg:w-[42px]"
            />
          ...
          <VideoCloseButton
            ...
            isVideoModalClose={isVideoModalClose}
          />
        </div>
        <div>
          {isVimeoSrc ? (
